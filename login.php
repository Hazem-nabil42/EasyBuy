<?php
// Start the session
session_start();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Login</title>
    <link rel="stylesheet" href="../../style/login.css">
</head>
<body>
    <div class="container">
        <div class="overlay" id="overlay">
          <div class="sign-in" id="sign-in">
            <h1>Welcome Back!</h1>
            <p>To keep connected with us please login with your personal info</p>
            <button class="switch-button" id="slide-right-button">Sign In</button>
          </div>
          <div class="sign-up" id="sign-up">
            <h1>Hello, Friend!</h1>
            <p>Enter your personal details and start a journey with us</p>
            <button class="switch-button" id="slide-left-button">Sign Up</button>
          </div>
        </div>
        <div class="form">
          <div class="sign-in" id="sign-in-info">
            <h1>Sign In</h1>
            <!-- <div class="social-media-buttons">
              <div class="icon">
                <svg viewBox="0 0 24 24">
                  <path fill="#000000" d="M17,2V2H17V6H15C14.31,6 14,6.81 14,7.5V10H14L17,10V14H14V22H10V14H7V10H10V6A4,4 0 0,1 14,2H17Z" />
              </svg>
              </div>
              <div class="icon">
              <svg viewBox="0 0 24 24">
                  <path fill="#000000" d="M23,11H21V9H19V11H17V13H19V15H21V13H23M8,11V13.4H12C11.8,14.4 10.8,16.4 8,16.4C5.6,16.4 3.7,14.4 3.7,12C3.7,9.6 5.6,7.6 8,7.6C9.4,7.6 10.3,8.2 10.8,8.7L12.7,6.9C11.5,5.7 9.9,5 8,5C4.1,5 1,8.1 1,12C1,15.9 4.1,19 8,19C12,19 14.7,16.2 14.7,12.2C14.7,11.7 14.7,11.4 14.6,11H8Z" />
              </svg>
              </div>
              <div class="icon">
              <svg viewBox="0 0 24 24">
                <path fill="#000000" d="M21,21H17V14.25C17,13.19 15.81,12.31 14.75,12.31C13.69,12.31 13,13.19 13,14.25V21H9V9H13V11C13.66,9.93 15.36,9.24 16.5,9.24C19,9.24 21,11.28 21,13.75V21M7,21H3V9H7V21M5,3A2,2 0 0,1 7,5A2,2 0 0,1 5,7A2,2 0 0,1 3,5A2,2 0 0,1 5,3Z" />
              </svg>
              </div>
            </div> -->
            <p class="small">or use your email account:</p>
            <form id="sign-in-form">      
              <input type="text" id="signIn_username" placeholder = "Username"/><br>
              <span id="error-log"></span>
              <input type="password" id="signIn_password" placeholder="Password" />
              <!-- <p class="forgot-password">Forgot your password?</p> -->
            </form>
            <button class="control-button in" onclick="signIn()">Sign In</button>
          </div>
          <div class="sign-up" id="sign-up-info">
            <h1>Create Account</h1>
            <!-- <div class="social-media-buttons">
              <div class="icon">
                <svg viewBox="0 0 24 24">
                  <path fill="#000000" d="M17,2V2H17V6H15C14.31,6 14,6.81 14,7.5V10H14L17,10V14H14V22H10V14H7V10H10V6A4,4 0 0,1 14,2H17Z" />
              </svg>
              </div>
              <div class="icon">
              <svg viewBox="0 0 24 24">
                  <path fill="#000000" d="M23,11H21V9H19V11H17V13H19V15H21V13H23M8,11V13.4H12C11.8,14.4 10.8,16.4 8,16.4C5.6,16.4 3.7,14.4 3.7,12C3.7,9.6 5.6,7.6 8,7.6C9.4,7.6 10.3,8.2 10.8,8.7L12.7,6.9C11.5,5.7 9.9,5 8,5C4.1,5 1,8.1 1,12C1,15.9 4.1,19 8,19C12,19 14.7,16.2 14.7,12.2C14.7,11.7 14.7,11.4 14.6,11H8Z" />
              </svg>
              </div>
              <div class="icon">
              <svg viewBox="0 0 24 24">
                <path fill="#000000" d="M21,21H17V14.25C17,13.19 15.81,12.31 14.75,12.31C13.69,12.31 13,13.19 13,14.25V21H9V9H13V11C13.66,9.93 15.36,9.24 16.5,9.24C19,9.24 21,11.28 21,13.75V21M7,21H3V9H7V21M5,3A2,2 0 0,1 7,5A2,2 0 0,1 5,7A2,2 0 0,1 3,5A2,2 0 0,1 5,3Z" />
              </svg>
              </div>
            </div> -->
            <p class="small">or use your email for registration:</p>
            <form id="sign-up-form">
                <input type="text" placeholder="Name" name="Name"  id="signUp_name"  /><br>
                <span id="error"></span>
                <input type="email" placeholder="Email" name="Email" id="signUp_email" /><br>
                <span id="error1"></span>
                <input type="password" placeholder="Password" name="Password"  id="signUp_password"  /><br>
                <span id="error2"></span>
                <input type="text"  placeholder="Address" name="Address" id="signUp_address" /> <br>
                <span id="error3"></span>
                <input type="text" placeholder="Phone" name="Phone" id="signUp_phone" /> <br>
                <span id="error4"></span>
                
            </form>
            <button class="control-button up" onclick="signUp()">Sign Up</button>

          </div>
        </div>
      </div>
      <script src="Frontend/JS/jquery.min.js"></script>
      <script defer src="Frontend/JS/login.js"></script>
      <script>
        function signIn() {
          let x = document.forms["sign-in-form"]["signIn_username"].value;
          if (x == "") {
              document.getElementById("error-log").innerHTML = "Please enter a username";
              document.getElementById("error-log").setAttribute("style", "color: red;");
                return false; 
            }
            var dataToSend = {
                'username' : $('#signIn_username').val() ,
                'password' : $('#signIn_password').val()
            }
            $.ajax({
                type: "POST",
                url: "login-control.php",
                data: dataToSend, 
                success: function(res) {
                    if(res == "1"){
                        $(location).prop('href', 'http://localhost/index.php')
                    }
                }
            });
        }

        // sign up
        function signUp() {
            let x = document.forms["sign-up-form"]["Name"].value;
            let y = document.forms["sign-up-form"]["Password"].value;
            let z = document.forms["sign-up-form"]["Email"].value;
            let a = document.forms["sign-up-form"]["Address"].value;
            let b = document.forms["sign-up-form"]["Phone"].value;
            
            if (x == "") {
              document.getElementById("error").innerHTML = "Please enter your name";
              document.getElementById("error").setAttribute("style", "color: red;");
                return false; 
            }
            else if(z== ""){
              document.getElementById("error1").innerHTML = "Please enter your email";
              document.getElementById("error1").setAttribute("style", "color: red;");
                return false;
            }
            else if(y== ""){
              document.getElementById("error2").innerHTML = "Please enter your password";
              document.getElementById("error2").setAttribute("style", "color: red;");
                return false;
            }
             if(a == ""){
              document.getElementById("error3").innerHTML = "Please enter your Address";
              document.getElementById("error3").setAttribute("style", "color: red;");
                return false;
            }
            else if(b == ""){
              document.getElementById("error4").innerHTML = "Please enter your phone number";
              document.getElementById("error4").setAttribute("style", "color: red;");
                return false;
            }
            
            
            var dataToSend = {
                'username' : $('#signUp_name').val() ,
                'password' : $('#signUp_password').val() ,
                'email' : $('#signUp_email').val() ,
                'address' : $('#signUp_address').val() ,
                'phone' : $('#signUp_phone').val()

            }
            $.ajax({
                type: "POST",
                url: "signup-control.php",
                data: dataToSend, 
                success: function(res) {
                    if(res == "1"){
                        $(location).prop('href', 'http://localhost/index.php')
                    }
                }
            });
           
        }
        
      
    
      </script>
</body>
</html>